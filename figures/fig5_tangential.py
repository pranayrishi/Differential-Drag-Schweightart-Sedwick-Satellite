"""
Figure 5 — Validation: tangential force only (Case C). KEY FIGURE.

delta_fx = 0, delta_fy = 1e-7 m/s^2.

This is the main result:
  - Shao deviates from numerical (secular error, especially in delta_y)
  - Traub matches numerical to machine precision
"""
...
def plot_figure_5(n, c, T, y0, t_eval):
    """
    Compare numerical, Shao, and Traub for Case C (tangential force only).
    ...
    ax.set_title(
        r'Case C: Tangential Force Only ($\delta f_x=0$, $\delta f_y = 10^{-7}$ m/s$^2$)',
        fontsize=11)
