"""
Figure 4 — Validation: radial force only (Case B).

delta_fx = 1e-7 m/s^2, delta_fy = 0.
Both Shao and Traub agree with numerical (Shao is correct when fy=0).
"""
...
def plot_figure_4(n, c, T, y0, t_eval):
    """
    Compare numerical, Shao, and Traub for Case B (radial force only).
    ...
    ax.set_title(r'Case B: Radial Force Only ($\delta f_x = 10^{-7}$ m/s$^2$, $\delta f_y = 0$)',
                 fontsize=11)
